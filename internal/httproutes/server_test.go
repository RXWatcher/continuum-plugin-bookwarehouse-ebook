...
// A malformed method from the untrusted RPC payload must not panic.
...
func TestHandle_NotConfigured(t *testing.T) {
	s := NewServer()
	resp, err := s.Handle(context.Background(), &pluginv1.HandleHTTPRequest{Method: "GET", Path: "/"})
	if err != nil {
		t.Fatalf("Handle: %v", err)
	}
